...
const CurveDecoration: React.FC<CurveDecorationProps> = ({ 
  ...
  // Determine height based on prop - reduced mobile heights to prevent overlapping
  const heightValues = {
    sm: { desktop: '80px', mobile: '30px' },
    md: { desktop: '120px', mobile: '40px' },
    lg: { desktop: '160px', mobile: '60px' }
  };
  ...
  return (
    <div 
      ...
      style={{ 
        height: 'clamp(' + mobileHeight + ', 8vh, ' + desktopHeight + ')',
        maxHeight: '12vh' // Reduced from 15vh for better mobile display
      }}
    >
      {position === 'top' ? (
        <svg 
